...
class HomeController extends Controller
{
    ...
    public function nseIndexStockList(Request $request)
    {
        ...
        $sectorIDData = StockDetails::select('sector_index')
            // ->where('sector_index', '<>', 'NA')
            ->groupBy('sector_index')
            ->orderBy('sector_index', 'asc')
            ->get();
        $watchListList = [];
        $stockConditions = '1=1';
        if($stock_name != null){
        ...
        foreach($sectorIDData as $sectorID){
            $sectorIDstocks = DB::table('s_stock_details')
                ...
                ->where('s_stock_daily_price_data.date', $today)
                ->where('s_stock_symbols.is_active', true)
                ->where('s_stock_details.sector_index', $sectorID->sector_index)
            ...
            if(count($sectorIDstocks)>0){
                $watchListList[str_replace([' ', '-','&'], '_', $sectorID->sector_index)]['name'] = $sectorID->sector_index;
                $watchListList[str_replace([' ', '-','&'], '_', $sectorID->sector_index)]['stock_list'] = $sectorIDstocks;
            }
        }
    ...
    public function indexDetailView(Request $request)
    {
        ...
        $stockDailyPriceRecords = StockDailyPriceData::where('date', $todayDate)
            // ->whereHas('sector_index_all', $index_name)
            ->whereRaw("CONCAT(',', sector_index_all, ',') LIKE ?", ["%,$index_name,%"])
            // ->whereRaw("FIND_IN_SET(?, REPLACE(sector_index_all, ' ', ''))", [$index_name])
            ->get();
